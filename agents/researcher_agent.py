...
try:
    ...
    class ChatOpenRouter:
         def __init__(self, *args, **kwargs):
            raise ImportError(
                "langchain-community and openrouter are not installed. Install with: pip install langchain-community llm-openrouter")
...
try:
    ...
    logger.warning(
        "VisualAssetCollector not available, visual asset collection will be skipped")
...
class ResearcherAgent:
    ...
    def __init__(
        ...
        self.llm_preference = llm_preference or [
        ...
        if not self.llm:
             raise TopicAnalysisError(
                 "Could not initialize any LLM. Please check API keys and provider preferences.")
    ...
    def _initialize_groq_llm(self) -> Optional[BaseChatModel]:
        ...
        if self.groq_api_key and GROQ_AVAILABLE:
            try:
                logger.info("Initializing LLM with Groq (deepseek r1)")
                return ChatGroq(
    ...
    def _initialize_openrouter_llm(self) -> Optional[BaseChatModel]:
        ...
        if self.openrouter_api_key and OPENROUTER_AVAILABLE:
            try:
                logger.info("Initializing LLM with OpenRouter (google/gemini-2.5-flash-preview:thinking)")
                return ChatOpenRouter(
                    model_name="google/gemini-2.5-flash-preview:thinking",  # Specify model
    ...
    def _get_fallback_order(self, start_provider: str) -> List[str]:
        ...
        try:
            ...
            return self.llm_preference[start_index:] + \
                self.llm_preference[:start_index]
        except ValueError:
    ...
    async def search_citations(self, query: str, count: int = 5) -> List[Dict[str, Any]]:
        ...
        if not self.brave_api_key:
            logger.warning(
                "Brave API key not available. Using mock citations.")
            # Return mock data without making any external calls
            return [{
                # Corrected mock data generation
                'title': f'Mock citation for "{query}" {i + 1}',
        ...
        try:
            ...
            for result in results:
                citation = {
                    ...
                    'source': result.get('profile', {}).get('name', 'Unknown Source'),
                    'date': datetime.now(timezone.utc).isoformat()
                }
        ...
        **kwargs
        ...
        for provider_name in fallback_order:
            ...
            try:
                ...
                # Return result and successful provider info
                return result, provider_name, llm_instance

            # Catch specific recoverable errors
            except (OpenAIRateLimitError, OpenAIAuthenticationError) as e:
    ...
    def calculate_readiness_score(
        self, metadata: Dict[str, Any], research_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        # Initialize dictionaries to track component percentages and missing
        # essentials
        ...
        if challenges_count >= challenges_min:
            ...
            if challenges_count > challenges_target:
                bonus_factor = min(1.0, (challenges_count - challenges_target) / (challenges_max - challenges_target))
        ...
        if pro_arguments_count >= pro_arguments_min:
            ...
            if pro_arguments_count > pro_arguments_target:
                bonus_factor = min(1.0, (pro_arguments_count - pro_arguments_target) / (pro_arguments_max - pro_arguments_target))
        ...
        if counter_arguments_count >= counter_arguments_min:
            ...
            if counter_arguments_count > counter_arguments_target:
                bonus_factor = min(1.0, (counter_arguments_count - counter_arguments_target) / (counter_arguments_max - counter_arguments_target))
        ...
        # Weights for current, transitions, future
        paradigm_weights = [0.5, 0.25, 0.25]
        ...
        # Solution analogies score - Can reuse generated count or add separate
        # logic
        ...
        # Apply grade cap based on critical missing components and minimum
        # thresholds
    ...
    def _evaluate_solution_nuance(
        self, research_data: Dict[str, Any]) -> float:
        """
        ...
        for challenge in challenges:
            if isinstance(challenge, dict) and 'solution' in str(
                challenge).lower():
    ...
    def _evaluate_audience_benefit_clarity(
        self, research_data: Dict[str, Any]) -> float:
        """
        ...
        # Check for explicit connections between solutions and audience
        # segments
        audience = research_data.get('audience', {})
    ...
    def generate_research_report(
        ...
        if system_name != 'Unknown System':
            ...
            if challenges:
                ...
                for i, challenge_data in enumerate(challenges[:5], 1):
                    # challenge_data might now be {'challenge': ..., 'sources':
                    # ..., 'components': ...}
        ...
        # Assuming description is not directly in solution_data anymore, get
        # from metadata or args if needed
        ...
        if solution_description != 'No description available':
            ...
            if pro_args_data:
                for i, arg_data in enumerate(pro_args_data[:5], 1):
                    ...
                    # Handle if inner structure is just a string (fallback)
                    else:
    ...
    def save_research_results(
        ...
        try:
            ...
            blog_id = self.db_client.db.blogs.update_one({"title": blog_title}, {"$set": {"current_version": version}, "$push": {"versions": blog_doc["versions"][0]}}, upsert=True).upserted_id
